import { isFlagYes, flagBorderHex } from '../lib/flags';
...
export function FeatureCard({ feature, onClick, stretched }: Props) {
  ...
  const flagHex = flagBorderHex(feature.flagColor);
  ...
  return (
    <button
      ...
      } ${flagged ? '' : 'theme-task-solid'}`}
      style={{
        backgroundColor: 'var(--app-task-bg)',
        ...(flagged ? { border: `2px dashed ${flagHex}` } : undefined),
      }}
    >
